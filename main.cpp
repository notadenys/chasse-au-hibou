...
void handle_startscreen_events(GUI* gui, Sound* sound, SDL_Event* event, bool* gameover, bool* continueStartscreen, bool* endgame)
{
    while (SDL_PollEvent(event))
    {
        switch (event->type)
...
void startscreen(Map* map, GUI* gui, Sound* sound, bool* gameover, SDL_Renderer *renderer, bool* endgame)
{
    ...
    {
        SDL_Event event;
        handle_startscreen_events(gui, sound, &event, gameover, &continueStartscreen, endgame);
    }
}
